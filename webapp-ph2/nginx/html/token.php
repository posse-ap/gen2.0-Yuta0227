...
if(!isset($_SESSION['start'])){
...
}elseif(isset($_GET['delete_id'])&&isset($_GET['reject_reason'])&&isset($_GET['date'])){
    // echo 1;
    //却下
    ...
    $post_fields = [
        "channel" => "#削除依頼",
        "text" => $_GET['date']."に".$_SESSION['user'][0]['user_name']."が投稿".$delete_id."を".$delete_reason."の理由で削除依頼しました。確認お願いします。",
        "as_user" => true
    ];
}elseif(isset($_GET['contents'])&&isset($_GET['language'])&&isset($_GET['hours'])&&isset($_GET['date'])){
    // echo 4;
    ...
    $post_fields = [
        "channel" => "#勉強報告",
        "text" => $_GET['date']."に".$_SESSION['user'][0]['user_name']."が".$language."を".$contents."で".$hours."時間勉強しました。",
        "as_user" => true
    ];
};
